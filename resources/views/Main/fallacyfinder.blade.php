...
@section('content')
<div class="max-w-4xl mx-auto" id="fallacy-finder-page" data-soal-id="{{ $soal->id_soal ?? '' }}">
    ...
    <div class="bg-brand rounded-2xl p-8 text-white mb-6">
        <h2 class="text-2xl font-black mb-2">Topik : {{ $soal->topik ?? 'Latihan Analisis' }}</h2>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-6">
        <p class="text-slate-700 text-sm leading-relaxed font-semibold">
            "{{ $soal->isi_soal ?? 'Teks studi kasus belum tersedia saat ini.' }}"
        </p>
    </div>
    ...
    {{-- Tombol Kirim --}}
    <button id="kirim-fallacy-finder" class="w-full bg-brand text-white font-bold py-4 rounded-xl text-lg hover:bg-brand-dark transition shadow-md">
        Kirim
    </button>
</div>
@endsection